...
export default function Lightbox({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentional reset on artworkIndex change
  useEffect(() => {
    setPhotoIndex(0);
  }, [artworkIndex]);
  ...
  const prevArtwork = () =>
    setArtworkIndex((prev) => (prev - 1 + artworks.length) % artworks.length);
  const nextArtwork = () =>
        ...
        <div className="relative flex-1 bg-charcoal flex items-center justify-center min-h-[300px] md:min-h-[500px]">
          ...
          {photos.length > 1 && (
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
              {photos.map((_photo, i) => (
                <button
                  // biome-ignore lint/suspicious/noArrayIndexKey: photos array has no stable ids
                  key={`photo-dot-${i}`}
                  type="button"
