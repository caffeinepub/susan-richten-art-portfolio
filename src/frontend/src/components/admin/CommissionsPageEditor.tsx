...
import type { FAQItem, ProcessStep } from "../../contexts/CMSContext";
...
export default function CommissionsPageEditor() {
  ...
  const handleSave = () => {
    updateSiteSettings({
      commissionHeroText: heroText,
      commissionPricingText: pricingText,
    });
    setIsDirty(false);
    setSaved(true);
  ...
  const addStep = () => {
    ...
    const nextId =
      processSteps.length > 0
        ? Math.max(...processSteps.map((s) => s.id)) + 1
        : 1;
  ...
  const addFAQ = () => {
    ...
    const nextId =
      faqItems.length > 0 ? Math.max(...faqItems.map((f) => f.id)) + 1 : 1;
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-charcoal/60">
          Edit the Commissions page content.
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          <label
            htmlFor="commission-hero-text"
            className="block text-sm font-medium text-charcoal mb-1"
          >
            Hero Text
          </label>
          <textarea
            id="commission-hero-text"
            value={heroText}
            onChange={(e) => {
        ...
        <div>
          <label
            htmlFor="commission-pricing-text"
            className="block text-sm font-medium text-charcoal mb-1"
          >
            Pricing Text
          </label>
          <textarea
            id="commission-pricing-text"
            value={pricingText}
            onChange={(e) => {
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            onClick={handleSave}
            data-ocid="commissions.save_button"
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-2">
          <input
            ...
            value={newStep.title}
            onChange={(e) =>
              setNewStep((p) => ({ ...p, title: e.target.value }))
            }
          ...
          <input
            ...
            onChange={(e) =>
              setNewStep((p) => ({ ...p, description: e.target.value }))
            }
