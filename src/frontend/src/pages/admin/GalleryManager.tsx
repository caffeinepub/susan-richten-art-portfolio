...
import { type Artwork, useCMS } from "../../contexts/CMSContext";

export default function GalleryManager() {
  const { artworks, addArtwork, updateArtwork, deleteArtwork, isLoading } =
    useCMS();
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="font-heading text-4xl text-charcoal mb-1">
            Gallery Manager
          </h1>
      ...
      {selectedIds.size > 0 && (
        <div className="mb-4 flex flex-wrap items-center gap-2 p-3 bg-charcoal text-beige rounded-sm">
          <span className="font-body text-sm mr-2">
      ...
      {showDeleteConfirm && (
        <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-sm flex items-center justify-between gap-4">
          <p className="font-body text-sm text-red-700">
            Delete {selectedIds.size} artwork{selectedIds.size > 1 ? "s" : ""}?
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-beige-dark">
              ...
              <th className="px-4 py-3 text-left font-body text-xs tracking-widest uppercase text-charcoal-muted w-8" />
              <th className="px-4 py-3 text-left font-body text-xs tracking-widest uppercase text-charcoal-muted">
                Artwork
              </th>
              <th className="px-4 py-3 text-left font-body text-xs tracking-widest uppercase text-charcoal-muted hidden md:table-cell">
                Category
              </th>
              <th className="px-4 py-3 text-left font-body text-xs tracking-widest uppercase text-charcoal-muted hidden lg:table-cell">
                Status
              </th>
              <th className="px-4 py-3 text-left font-body text-xs tracking-widest uppercase text-charcoal-muted">
                Actions
          ...
          <tbody>
            {isLoading
              ? Array.from({ length: 5 }, (_, i) => `sk-row-${i}`).map(
                  (key) => (
                    <tr key={key} className="border-b border-beige-dark">
              ...
              : sorted.map((artwork) => (
                  <tr
                    key={artwork.id}
                    className="border-b border-beige-dark last:border-0 hover:bg-beige/30 transition-colors"
                  >
                    <td className="px-4 py-3">
                      <button
                        type="button"
                    ...
                    <td className="px-4 py-3">
                      <GripVertical
                        size={16}
                        className="text-charcoal-muted cursor-grab"
                      />
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
