...
import { type CommissionInquiry, useCMS } from "../../contexts/CMSContext";
...
export default function CommissionsManager() {
  const { commissionInquiries, updateCommissionInquiryStatus, isLoading } =
    useCMS();
      ...
      {isLoading ? (
        <div className="bg-white shadow-xs overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {Array.from({ length: 4 }, (_, i) => `sk-row-${i}`).map((key) => (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-beige-dark">
                ...
                <th className="px-4 py-3 text-left font-body text-xs tracking-widest uppercase text-charcoal-muted hidden xl:table-cell">
                  Description
                </th>
              </tr>
            </thead>
