...
export default function Dashboard() {
  ...
  const newInquiries = commissionInquiries.filter(
    (i) => i.status === "new",
  ).length;
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {isLoading
          ? Array.from({ length: 4 }, (_, i) => `sk-card-${i}`).map((key) => (
              <Card key={key}>
          ...
          <CardContent>
            {isLoading ? (
              <div className="space-y-3">
                {Array.from({ length: 3 }, (_, i) => `sk-row-${i}`).map(
                  (key) => (
                    <div
                      key={key}
              ...
              <div className="space-y-3">
                {recentInquiries.map((inquiry) => (
                  ...
                  >
                    ...
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${statusColor(inquiry.status)}`}
                    >
                      {inquiry.status}
                    </span>
                  </div>
