...
function toBackendCommissionInquiry(
  c: FrontendCommissionInquiry,
): CommissionInquiry {
  return {
    id: BigInt(c.id),
    name: c.name,
...
function fromBackendCommissionInquiry(
  c: CommissionInquiry,
): FrontendCommissionInquiry {
  return {
    id: Number(c.id),
    name: c.name,
    email: c.email,
...
function fromBackendCareerMilestone(
  m: CareerMilestone,
): FrontendCareerMilestone {
  return {
    id: Number(m.id),
    year: m.year,
    event: m.event,
...
function toBackendSiteSettings(s: FrontendSiteSettings): SiteSettings {
  return {
    ...
    socialPlatforms: s.socialPlatforms.map(
      (p): SocialPlatform => ({
        name: p.name,
        url: p.url,
        icon: p.icon,
...
export function useCMSBackend() {
  ...
  async function loadAllCMSData(): Promise<FrontendAllCMSData | null> {
    ...
    try {
      ...
      return {
        ...
        pressMentions: Array.from(data.pressMentions).map(
          fromBackendPressMention,
        ),
        commissionInquiries: Array.from(data.commissionInquiries).map(
          fromBackendCommissionInquiry,
        ),
        contactInquiries: Array.from(data.contactInquiries).map(
          fromBackendContactInquiry,
        ),
        notifications: Array.from(data.notifications).map(
          fromBackendNotification,
        ),
        blogPosts: Array.from(data.blogPosts).map(fromBackendBlogPost),
        faqItems: Array.from(data.faqItems).map(fromBackendFAQItem),
        processSteps: Array.from(data.commissionProcessSteps).map(
          fromBackendProcessStep,
        ),
        careerMilestones: Array.from(data.careerMilestones).map(
          fromBackendCareerMilestone,
        ),
        siteSettings: data.siteSettings
          ? fromBackendSiteSettings(data.siteSettings)
          : null,
        navigationItems: Array.from(data.navigationItems).map(
          fromBackendNavigationItem,
        ),
      };
    } catch (err) {
  ...
  async function saveArtworks(artworks: FrontendArtwork[]): Promise<void> {
    if (!actor) {
      console.error("[useCMSBackend] saveArtworks: actor not ready");
      return;
    }
    try {
  ...
  async function saveTestimonials(
    ...
    if (!actor) {
      console.error("[useCMSBackend] saveTestimonials: actor not ready");
      return;
    }
    try {
  ...
  async function savePressMentions(
    pressMentions: FrontendPressMention[],
  ): Promise<void> {
    if (!actor) {
      console.error("[useCMSBackend] savePressMentions: actor not ready");
      return;
    }
    try {
  ...
  async function saveCommissionInquiries(
    inquiries: FrontendCommissionInquiry[],
  ): Promise<void> {
    if (!actor) {
      console.error("[useCMSBackend] saveCommissionInquiries: actor not ready");
      return;
    }
    try {
      await actor.setCommissionInquiries(
        inquiries.map(toBackendCommissionInquiry),
      );
    } catch (err) {
  ...
  async function saveContactInquiries(
    inquiries: FrontendContactInquiry[],
  ): Promise<void> {
    if (!actor) {
      console.error("[useCMSBackend] saveContactInquiries: actor not ready");
      return;
    }
    try {
  ...
  async function saveNotifications(
    ...
    if (!actor) {
      console.error("[useCMSBackend] saveNotifications: actor not ready");
      return;
    }
    try {
  ...
  async function saveBlogPosts(blogPosts: FrontendBlogPost[]): Promise<void> {
    if (!actor) {
      console.error("[useCMSBackend] saveBlogPosts: actor not ready");
      return;
    }
    try {
  ...
  async function saveFAQItems(faqItems: FrontendFAQItem[]): Promise<void> {
    if (!actor) {
      console.error("[useCMSBackend] saveFAQItems: actor not ready");
      return;
    }
    try {
  ...
  async function saveProcessSteps(steps: FrontendProcessStep[]): Promise<void> {
    if (!actor) {
      console.error("[useCMSBackend] saveProcessSteps: actor not ready");
      return;
    }
    try {
  ...
  async function saveCareerMilestones(
    milestones: FrontendCareerMilestone[],
  ): Promise<void> {
    if (!actor) {
      console.error("[useCMSBackend] saveCareerMilestones: actor not ready");
      return;
    }
    try {
  ...
  async function saveSiteSettings(
    ...
    if (!actor) {
      console.error("[useCMSBackend] saveSiteSettings: actor not ready");
      return;
    }
    try {
  ...
  async function saveNavigationItems(
    ...
    if (!actor) {
      console.error("[useCMSBackend] saveNavigationItems: actor not ready");
      return;
    }
    try {
