...
export default {
  meta: {
    ...
    docs: {
      description:
        "Ensure InternetIdentityProvider is present when using Internet Identity hooks",
      recommended: true,
    },
    messages: {
    ...
    return {
        ...
        if (isInternetIdentityHookSource(node.source.value)) {
          for (const spec of node.specifiers) {
            if (
              ...
              identityHooks.includes(spec.imported.name)
            ) {
      ...
      CallExpression(node) {
        if (
          ...
          importedHooks.has(node.callee.name)
        ) {
          hookUsages.push({
            node: node,
